...
inline ShotResult FireRay(Vector3 origin, Vector3 direction, float maxRange,
  ...
  for (int i = 0; i < MAX_PAWNS; i++) {
    ...
    if (!p.alive || i == shooterID)
      continue;
    // Don't shoot through smoke for bots (player shooting through smoke is
    // allowed)
...
inline void WeaponFire(Pawn &shooter, World &world, bool isADS,
  ...
  if (audio)
    audio->PlayShoot(ws.id);

  const WeaponStats &st = ws.stats();
  ws.ammoMag--;
  ws.fireCooldown = 60.0f / st.fireRateRPM;

  float spread = st.spreadRad;
  if (ws.id == WeaponID::SNIPER && !isADS) {
    spread += 0.3f; // Severe penalty for no-scope AWP
  } else if (isADS) {
    spread *= st.adsSpreadMult;
  }

  // Determine shooter's speed for movement penalty
  float speed = sqrtf(shooter.velocity.x * shooter.velocity.x +
                      shooter.velocity.z * shooter.velocity.z);

  // Add movement spread penalty (CS 1.6 style: penalty scales with speed)
  if (!shooter.onGround) {
    spread += 0.4f; // Jumping penalty
  } else if (speed > 1.0f) {
    spread += (speed / PLAYER_SPEED) * 0.10f; // Movement penalty
  }
...
inline bool ThrowUtility(Pawn &thrower, UtilityID type, World &world) {
  ...
  world.grenades.push_back(
      {type, thrower.eyePos(), vel, fuse, false, 0.0f, thrower.id});
  return true;
}